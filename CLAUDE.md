...
- **2026-05-06 (share-link env leak)**: Producer reported their share link rendering as `https://skitza-v2-web.vercel.app/join/giliasr` instead of the canonical `https://skitza.app/join/giliasr`. Root cause: the SidebarShareChip + dashboard share-URL builder threaded an env-driven `publicBaseUrl` (`NEXT_PUBLIC_SITE_URL` → `SITE_URL` → fallback) all the way down. With one of those env vars set to the Vercel preview host in production (or unset on a path that fell through to a `https://skitza-v2-web.vercel.app` literal in 7 different files), every producer's bio link silently pointed at the preview domain. Fix: introduced [`apps/web/src/lib/share/public-url.ts`](apps/web/src/lib/share/public-url.ts) — `PUBLIC_BRAND_ORIGIN = "https://skitza.app"` + `buildJoinUrl(slug)`. `app-shell.tsx` and `dashboard/page.tsx` now use the helper directly (no env reads). Lower-priority Vercel-URL fallbacks (sitemap, robots, metadataBase, email `SITE_URL`, Stripe `getSiteUrl`) updated to default to `https://skitza.app` for consistency, but kept env-overridable so local dev + Stripe-test redirects still work. **Rule going forward: any URL a producer might paste into their IG bio / socials / portfolio must be brand-canonical and NEVER env-driven.** Internal redirects (Stripe callbacks, magic links, email deep-links) legitimately need env overrides — those continue to use `getSiteUrl()` / `SITE_URL`. Easy grep for regressions: `grep -rn 'NEXT_PUBLIC_SITE_URL ?? .* ?? "https://skitza.app"' apps/web/src` should NEVER match for share-URL surfaces; if a future PR threads an env-driven base into a `/join/<slug>` builder, the test in `lib/share/__tests__/public-url.test.ts` (which env-sets `NEXT_PUBLIC_SITE_URL` to the Vercel preview host and asserts `buildJoinUrl` still returns `https://skitza.app/...`) won't catch it directly — pair with a source-grep test if this regresses.
