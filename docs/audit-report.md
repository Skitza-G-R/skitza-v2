...
- **2026-04-22 — /artist-welcome no-slug safety net** (Gili's 3rd manual-QA pass: screenshot showed him stuck on the orphan welcome *despite* the webhook-race fix above):
  - **Root cause discovered:** `/artist-welcome` (the no-slug page) was a bare Server Component with **zero** auth/role checking. Any authed user who landed there — via `(artist)/layout` redirecting on webhook-race, via browser back button, via reload, via a direct URL — saw the "ask a producer for a link" copy even if their `client_contacts` row actually existed. The server-action fix only helps if the user CLICKS the button on `/artist-welcome/<slug>`; it does nothing if they arrive at `/artist-welcome` (no slug) through any other path.
  - **Fix:** Added a `fetchUserRole` guard to the no-slug welcome page + a new pure `decideArtistWelcomeRedirect` helper. Authed artists → `/artist`. Authed producer-incomplete → `/onboarding`. Authed producer-complete → `/dashboard`. Only true orphans (no role resolved) render the welcome copy — and even they now get a "Refresh" link since their state is usually transient. Also added a small "Just signed up? Your workspace may still be loading." hint line to the orphan copy for the sub-second webhook-race window.
  - **TDD:** 5 new tests for `decideArtistWelcomeRedirect` pin the role → redirect contract. RED-verified by missing-module import error → GREEN after implementation. **621 tests pass** (up from 616).
  - **Files:** `apps/web/src/app/(artist-welcome)/artist-welcome/decide-redirect.ts` (new), `__tests__/decide-redirect.test.ts` (new, 5 tests), `page.tsx` (rewritten — Server Component with role guard).
- **2026-04-22 — killed the welcome splash entirely** (Gili's 4th manual-QA pass: *"i dont need this page its worthless, when i click the button it should take me straight to the app"*):
  - **Root cause insight:** even with the server-action-on-click fix, the button-click path had undiagnosable silent-failure modes (form-action `.bind()` flakiness, action not running, etc.). User never saw "success" — just ended up on /artist-welcome no-slug again. Without server-side observability (Sentry — roadmap S2.3), we couldn't tell why.
  - **Fix:** `apps/web/src/app/(artist-welcome)/artist-welcome/[slug]/page.tsx` is now a silent pass-through. On every visit it awaits `joinArtistWorkspace(slug)` server-side, which does the client_contacts upsert and redirects to /artist. No UI, no button, no click. User goes from Clerk's post-signup redirect → /artist, blink of nothing in between. `export const dynamic = "force-dynamic"` ensures the redirect runs every visit (not cached).
  - **Diagnostic logging added** to `joinArtistWorkspace` at entry, on every decision branch, and around the INSERT — surfaces in Vercel Function Logs so the next time someone reports "stuck on orphan welcome" we can trace the execution path. Marked to remove once Sentry lands.
  - **Verification:** typecheck ✅ / lint ✅ / 621 tests pass (action tests unchanged — logic is the same, invocation site moved from `<form>` to page), production build clean.
