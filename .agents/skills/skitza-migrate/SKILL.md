...
3. If the target is unclear, stop and ask Gili. If it is production, state that production is Neon project `skitza-v3` (`raspy-pine-96654399`) and get Gili's explicit approval for this exact run. Treat the old `skitza` project (`quiet-sun-92221754`) as read-only unless Gili explicitly changes that decision.
