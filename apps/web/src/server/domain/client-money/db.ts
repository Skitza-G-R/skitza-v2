...
/**
 * Loads all purchase-owned money rows directly on the HTTP client (SK-258: no
 * per-read transaction). Every query carries the producer boundary; the domain service verifies every
...
export function clientMoneyRepository(db: Db): ClientMoneyRepository {
  return {
    loadSnapshot: async (scope): Promise<ClientMoneySnapshot | null> => {
