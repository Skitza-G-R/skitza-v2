...
/**
 * Read every accepted-purchase row from one repeatable, read-only snapshot.
...
export function paymentLedgerReadRepository(db: Db): PaymentLedgerReadRepository {
  return {
    loadSnapshot: (scope) =>
      db.transaction(
        async (tx): Promise<PaymentLedgerReadSnapshot | null> => {
          ...
          const [
            ...
            tx
              ...
              .where(
                producerBoundary(purchaseInstallments.producerId, purchaseInstallments.purchaseId),
              )
            ...
            tx
              ...
              .where(
                producerBoundary(
                  purchaseCancellations.producerId,
                  purchaseCancellations.purchaseId,
                ),
              ),
            tx
              .select({
