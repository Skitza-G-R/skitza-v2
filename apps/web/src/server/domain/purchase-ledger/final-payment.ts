 ...
 * artist approves the final version inside Skitza. When that approval never
 * arrives the money is stuck for good — invisible to every "due now" reader,
 * not recordable, and not even waivable (`waiveInstallmentDebt` refuses an
 * untriggered installment).
 *
 * SK-269 unstuck imported work, where the client may never join and the songs
 * usually pre-date Skitza. SK-293 dropped that restriction, because a client
 * who bought through Skitza and then went quiet — approved over WhatsApp, paid
 * by bank transfer, never opened the app again — leaves the producer in the
 * identical dead end. 50/50 is a milestone plan: the money is owed when the
 * work is done, and the producer is the one who knows it is done.
 *
 * Permission is not prominence. This module answers only "may the producer
 * settle this half?". The Payments tab still offers the standalone "the work is
 * done" card on imported work alone, so a healthy in-flight project is never
 * nudged to collect early; everywhere else the answer is reached deliberately,
 * by recording money that already arrived.
 *
 * This is the single source of that rule. The Payments tab uses it to decide
 * whether to offer the control, and `requestFinalPayment` uses the same answer
 * to decide whether to honour the request, so a hidden button and a refused
 * mutation can never disagree.
 */
