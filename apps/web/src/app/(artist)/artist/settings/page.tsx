  ...
  return (
    <div className="reveal-up space-y-4 pb-12">
      <header className="px-1">
        <h1 className="font-display text-[30px] leading-none font-extrabold tracking-[-0.035em] text-[rgb(var(--fg-default))]">
          Settings
      ...
      >
        ...
        <div className="flex items-center gap-3">
          {user?.imageUrl ? (
            ...
            <div
              className="font-display flex h-14 w-14 items-center justify-center rounded-full text-xl font-extrabold text-[rgb(var(--fg-onsidebar))]"
              style={{
                background:
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="truncate text-xs text-[rgb(var(--fg-muted))]">{email}</p>
            {joined ? (
      ...
      >
        <h2
          ...
          className="px-4 pt-4 pb-1 font-mono text-[0.66rem] font-semibold tracking-wider text-[rgb(var(--fg-muted))] uppercase"
        >
          Connected producers · {studios.length}
        </h2>
        {studios.length === 0 ? (
          <p className="px-4 pt-2 pb-4 text-sm text-[rgb(var(--fg-secondary))]">
            No producers yet. Once a producer invites you, they'll show up here.
          </p>
        ) : (
          <ul className="divide-y divide-[rgb(var(--border-subtle))] border-t border-[rgb(var(--border-subtle))]">
            {studios.map((s) => (
              <li key={s.producerId} className="flex items-center gap-3 px-4 py-2.5">
                {s.logoUrl ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img
                    src={s.logoUrl}
                  ...
                  <div
                    className="font-display flex h-8 w-8 items-center justify-center rounded-[var(--radius-sm)] text-xs font-extrabold text-[rgb(var(--fg-onsidebar))]"
                    style={{
                      background:
                        "linear-gradient(135deg, rgb(var(--brand-primary)) 0%, rgb(var(--brand-copper)) 100%)",
                    }}
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="truncate text-[10.5px] text-[rgb(var(--fg-muted))]">{s.slug}</p>
                </div>
                <DisconnectProducerButton producerId={s.producerId} producerName={s.name} />
              </li>
            ))}
          </ul>
        )}
      ...
      <div className="px-1 pt-4 text-center">
        <Wordmark size={16} />
        <p className="mt-1 font-mono text-[10px] text-[rgb(var(--fg-muted))]">artist · v1.0</p>
      </div>
    </div>
  );
}
