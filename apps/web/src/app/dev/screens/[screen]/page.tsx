...
function Sk72RequestDevPreview({
  state,
}: {
  state: "pending" | "approved" | "declined" | "accepted";
}) {
  ...
  return (
    <main className="mx-auto w-full max-w-[1120px] px-4 py-6 sm:px-6 sm:py-10 lg:px-8">
      <PurchaseRequestReview
        id={DEV_REQUEST_ID}
        initialStatus={accepted ? "converted" : state}
        ...
        targetProjects={[
          ...
          },
          {
            ...
            lifecycleStatus: "waiting_for_payment",
            workflowStage: "brief",
            updatedAtIso: "2026-07-12T10:00:00.000Z",
          },
        ]}
        artistName="Maya Cohen"
        artistEmail="maya.long.email@example.invalid"
        productName={DEV_SK72_COMMERCIAL_SNAPSHOT.productOrOfferName}
        total="₪2,832"
        totalCaption={accepted ? "Accepted total" : "Proposal total"}
        submittedAt="Jul 20, 2026, 11:00 AM"
        reference="SK-72QA"
        brief="Keep the lead vocal intimate and leave space for a live string overdub. The chorus should feel wider without losing the close, human quality of the verses. Please keep the final instrumental clean enough for a live vocal performance and leave the string arrangement editable until the final review."
      >
...
export default async function DevScreenPage({ params }: Params) {
  ...
  switch (screen) {
    ...
    case "sk201-request-declined":
      return <Sk72RequestDevPreview state="declined" />;
