  ...
  return (
    <div className="min-w-0">
      ...
      <dd
        ...
        style={{
          color: danger ? "rgb(var(--fg-danger))" : "rgb(var(--fg-default))",
        }}
      >
...
function PurchaseLedger({ purchase }: { purchase: ClientMoneyPurchase }) {
  return (
    <section className="min-w-0 overflow-hidden rounded-[var(--radius-md)] border border-[rgb(var(--border-subtle))] bg-[rgb(var(--bg-background))]">
      <header className="min-w-0 px-3 py-3 sm:flex sm:items-start sm:justify-between sm:gap-4">
        <div className="min-w-0">
          <div className="flex min-w-0 flex-wrap items-center gap-2">
            <p className="font-mono text-[10px] font-bold tracking-[0.08em] text-[rgb(var(--fg-muted))]">
              {purchase.reference}
            </p>
            <StatusBadge value={purchase.lifecycleStatus} />
          </div>
          <h4 className="mt-1 text-[14px] font-bold break-words text-[rgb(var(--fg-default))]">
            {purchase.title}
          </h4>
        ...
        <div className="mt-2 shrink-0 sm:mt-0 sm:text-right">
          <p className="font-mono text-[16px] font-extrabold text-[rgb(var(--fg-default))] tabular-nums">
            {formatMoney(purchase.totalCents, purchase.currency, { withCents: true })}
          </p>
      ...
      <dl className="grid min-w-0 grid-cols-2 gap-px border-y border-[rgb(var(--border-subtle))] bg-[rgb(var(--border-subtle))] sm:grid-cols-5">
        <PurchaseAmount label="Subtotal" cents={purchase.subtotalCents} purchase={purchase} />
        <PurchaseAmount label="Tax" cents={purchase.taxCents} purchase={purchase} />
        <PurchaseAmount label="Total" cents={purchase.totalCents} purchase={purchase} />
        <PurchaseAmount label="Paid" cents={purchase.paidCents} purchase={purchase} />
        <PurchaseAmount
          label="Remaining"
      ...
      <div className="grid min-w-0 grid-cols-1 gap-3 p-3 xl:grid-cols-2">
        <div className="min-w-0">
            ...
            <ul className="mt-2 min-w-0 space-y-2">
              {purchase.proofs.map((proof) => (
                ...
                >
                  ...
                  {proof.rejectionNote ? (
                    <p className="mt-2 rounded-[var(--radius-sm)] bg-[rgb(var(--fg-danger)/0.08)] px-2 py-1.5 text-[11px] leading-relaxed break-words text-[rgb(var(--fg-danger))]">
                      {proof.rejectionNote}
                    </p>
                  ) : null}
                </li>
...
function PurchaseAmount({
  ...
  return (
    <div className="min-w-0 bg-[rgb(var(--bg-elevated))] px-2.5 py-2.5">
      <dt className="text-[9px] font-bold tracking-[0.08em] text-[rgb(var(--fg-muted))] uppercase">
        {label}
      </dt>
      <dd
        className="mt-0.5 font-mono text-[11.5px] font-bold break-words tabular-nums"
        style={{
          color: danger ? "rgb(var(--fg-danger))" : "rgb(var(--fg-default))",
        }}
      >
